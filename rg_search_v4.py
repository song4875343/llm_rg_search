...
class DocAgenticSearch:
    ...
    def _generate_regex(self, user_question: str, missing_info: str = "", history_queries: set = None, 
        ...
        # =======================================================
            ...
            请分析用户的提问，提取核心实体和属性，生成【一个】高覆盖率的 ripgrep 组合正则表达式。
            ...
            3. 泛化与并列：
               ...
               - 如有工程,法律,金融等专业术语请不要扩展不要调整 例如钢柱的长细比：'(钢柱|框架柱|柱).*?长细比'
    ...
    def _filter_and_expand(self, user_question: str, raw_hits: List[Dict], context: ContextWindow) -> bool:
        ...
        prompt = f"""你是一位极其严谨的规范审查专家。请为以下搜索命中的文档锚点进行打分（0-10分），判断其是否能回答用户问题。

        用户问题："{user_question}"

        ⚠️⚠️⚠️ 核心避坑指南（防“张冠李戴”陷阱）：
        规范文本中经常出现相同的【通用属性词】（如：间距、厚度、配筋率、宽高比、温度、期限等）。
        在打分前，你必须在脑海中将用户问题拆解为【核心主体】+【目标属性】。
        如果检索到的文本仅仅命中了【目标属性】，但描述的【核心主体】与用户提问**不一致**（例如：问A构件，答B构件；问地下，答地上；问商业建筑，答住宅建筑），这属于严重的误导信息，**必须触发一票否决，打 0-3 分！**

        评分标准：
        【9-10 分】精准匹配：【核心主体】完全一致，且直接给出了用户所需的【目标属性】的具体数值、公式、表格或明确规定。
        【7-8 分】高度相关：【核心主体】一致，虽未直接给出最终数值，但提供了关键背景要求，或指引了具体去查阅哪个表格/条文。
        【4-6 分】弱相关：提到了相关概念或主体，但属于宏观原则、定义说明，没有提供实质性解决问题的参数或规定。
        【0-3 分】致命不匹配（一票否决）：
        - 主体错位：文本讲的完全是另一个对象（哪怕属性词一模一样）。
        - 范围错位：适用场景完全不同。
        - 纯粹的关键词偶然碰撞，毫无逻辑关联。
        ...
        {{
          "scores": [
            {{
              "id": 0, 
              "reason": "提问主体是[某某]，本条文描述的主体是[另一个某某]，虽然都包含'[某属性]'，但适用对象完全错位，不具备参考价值。", 
              "score": 1
            }},
            {{
              "id": 1, 
              "reason": "本条文的主体与提问完全一致，且明确给出了要求的限值，完美匹配。", 
              "score": 9
            }}
          ]
        }}"""
