...
# ================= 辅助函数 =================
get_available_files = lambda search_dir: sorted(
    [
        f
...
cut_by_punctuation = lambda text: [
    s.strip()
    for s in re.findall(r"[^。！？.!?]+[。！？.!?]?", text.strip())
    if s.strip()
]
get_candidate_key = lambda item: (
    ("rg", item["file"], item["line_num"])
    if item["type"] == "rg"
    else ("chunk", item["file"], item["start_pos"])
)
...
def chunk_file(filepath: str, chunk_size: int = 512, overlap: int = 50) -> list:
    ...
    if current_chunk:
        chunks.append(
            {
                "content": "".join(sent for sent, _, _, _, _ in current_chunk),
                "file": filepath,
                "start_line": current_chunk[0][1],
                "end_line": current_chunk[-1][3],
                "start_pos": current_chunk[0][2],
...
def attach_bm25_scores(candidates: list, query: str) -> list:
    ...
    for idx, item in enumerate(candidates):
        item.update(
            bm25_score=score_by_index.get(idx, 0.0), score=score_by_index.get(idx, 0.0)
        )
    return candidates
...
def _keyword_bonus(content: str, broad_keywords: list, exact_keywords: list) -> float:
    ...
    return (0.5 if broad_hits == 2 else 1.0 if broad_hits >= 3 else 0.0) + (
        1.0
...
def _dedupe_candidates(candidates: list) -> list:
    rg_contents = {
        item["content"].strip().lower() for item in candidates if item["type"] == "rg"
    }
    ...
    for item in candidates:
        ...
        elif item["type"] == "chunk" and not any(
            rg in content or content in rg for rg in rg_contents
        ):
            deduplicated.append(item)
    return deduplicated
...
def _format_result(item: dict, context_lines: int) -> str:
    return (
        ...
        if item["type"] == "rg"
        else f"--- {os.path.basename(item['file'])}:行{item['start_line']}-{item['end_line']} [CHUNK] ---\n{item['content']}\n"
    )
    ...
    def _core():
        ...
        for filename in _target_files:
            if filepath := _find_target_path(search_dir, filename):
                ...
                yield from _emit(
                    stream, f"✅ 文件 {filename} 切分为 {len(chunks)} 个 chunks"
                )
        ...
        for item in all_candidates:
            item["keyword_bonus"] = _keyword_bonus(
                item["content"], broad_keywords, _exact_keywords
            )
            item["boosted_score"] = item["bm25_score"] + item["keyword_bonus"]
        bm25_sorted = sorted(
            all_candidates, key=lambda x: x["bm25_score"], reverse=True
        )
        boosted_sorted = sorted(
            all_candidates, key=lambda x: x["boosted_score"], reverse=True
        )
        dedup_keys = {
            get_candidate_key(item) for item in _dedupe_candidates(bm25_sorted)
        }
        bm25_dedup = [
            item for item in bm25_sorted if get_candidate_key(item) in dedup_keys
        ]
        boosted_dedup = [
            item for item in boosted_sorted if get_candidate_key(item) in dedup_keys
        ]
        ...
        yield from _emit(
            stream,
            f"📊 去重后: {len(dedup_keys)} 条 (RG优先), BM25取前{bm25_pick_count} + 修正取前{boosted_pick_count} -> Top-{len(top_items)}",
        )
...
def run_search(
    ...
    def _core():
        ...
        # 第一轮一定要检索，直接强制调用固定工具；这一步也不需要 thinking。
        first_round_kwargs = build_chat_kwargs(
            messages,
            tools=TOOLS,
        ...
        final_response = client.chat.completions.create(
            **build_chat_kwargs(messages_2, stream=stream, temperature=1)
        )
        if stream:
...
if __name__ == "__main__":
    # 非流式模式（默认）板的最小厚度
    run_search(
        query="扩展基础的宽高比", search_dir="./specs", top_k=30, context_lines=0
    )
